    ...
    EXCEPTION 
      WHEN OTHERS THEN
        ...
        IF v_retry_count >= v_max_retries THEN
          -- Log error and return null after max retries
          RAISE WARNING 'Failed to create audit log after % attempts: %', v_max_retries, SQLERRM;
          RETURN NULL;
        END IF;
...
-- Create function to handle failed audit logs
...
BEGIN
  ...
  -- Delete old failed logs after 30 days
...
-- Create cron job to process failed logs
SELECT cron.schedule(
  'process-failed-audit-logs',
  '*/5 * * * *', -- Every 5 minutes
  'SELECT process_failed_audit_logs();'
);