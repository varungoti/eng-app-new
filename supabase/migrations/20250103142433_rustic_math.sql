...
DROP POLICY IF EXISTS "health_check_read_policy_v3" ON _health;
DROP POLICY IF EXISTS "health_check_update_policy_v3" ON _health;

-- Create new policies
CREATE POLICY "health_check_read_policy_v4"
  ON _health FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "health_check_update_policy_v4"
