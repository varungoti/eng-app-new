            ...
            {showCloseButton && (
              <div className="absolute -top-[5px] left-1/2 transform -translate-x-1/2 w-[12px] h-[26px] flex items-center justify-center z-20">
                <button
                  ...
                  aria-label="Close"
                >
                  <Icon type="phosphor" name="CHEVRON_UP" className="text-muted-foreground/80" />
                </button>
  ...
  return (
    ...
    >
      <AnimatePresence mode="popLayout" initial={false}>
        ...
        >
          {loading ? (
            <Icon type="phosphor" name="ARROWS_CLOCKWISE" className="animate-spin size-3" />
          ) : (
            <span>{text}</span>
          )}
        </motion.span>
...
export function PopoverFormCutOutTopIcon({
  width = 44,
  height = 30,
}: {
  width?: number
...
// ... rest of the component (PopoverFormSuccess, icons, etc.) remains the same 